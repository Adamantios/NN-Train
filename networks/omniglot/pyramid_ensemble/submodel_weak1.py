...
def omniglot_pyramid_ensemble_submodel_weak1(input_shape=None, input_tensor=None, n_classes=None,
    ...
    # Block1.
    x = Conv2D(32, (3, 3), activation='elu', name='block1_conv1', kernel_regularizer=l2(weight_decay))(inputs)
    x = BatchNormalization(name='block1_batch-norm')(x)
    x = MaxPooling2D(pool_size=(2, 2), name='block1_pool')(x)
    x = Dropout(0.2, name='block1_dropout', seed=0)(x)

    # Block2
    x = Conv2D(32, (2, 2), activation='elu', name='block2_conv1', kernel_regularizer=l2(weight_decay))(x)
    ...
    model = Model(inputs, outputs, name='omniglot_pyramid_ensemble_submodel_weak1')
...
def omniglot_pyramid_ensemble_submodel_weak1_labels_manipulation(labels_array: ndarray) -> int:
    """
    ...
    """
    labels_array[labels_array > 812] = 812
    return 813
