...
def omniglot_complicated_ensemble_submodel5(input_shape=None, input_tensor=None, n_classes=None,
    ...
    # Create Submodel 1.
    model = Model(inputs, outputs, name='omniglot_complicated_ensemble_submodel5')
...
def omniglot_complicated_ensemble_submodel5_labels_manipulation(labels_array: ndarray) -> int:
    """
    ...
    """
    # Labels [1299 - 1622].
    labels_array[labels_array > 1622] = -1

    for i, label_i in enumerate(range(1299, 1623)):
        labels_array[labels_array == label_i] = i

    labels_array[labels_array == -1] = 324
