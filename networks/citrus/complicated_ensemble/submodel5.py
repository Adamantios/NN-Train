...
def citrus_complicated_ensemble_submodel5(input_shape=None, input_tensor=None, n_classes=None,
    ...
    # Block1.
    x = Conv2D(16, (3, 3), activation='relu', name='block1_conv1', kernel_regularizer=l2(weight_decay))(inputs)
    x = BatchNormalization(name='block1_batch-norm')(x)
    x = MaxPooling2D(pool_size=(2, 2), name='block1_pool')(x)
    x = Dropout(0.2, name='block1_dropout')(x)

    # Block2
    x = Conv2D(32, (3, 3), activation='relu', name='block2_conv1', kernel_regularizer=l2(weight_decay))(x)
    ...
    # Create Submodel 1.
    model = Model(inputs, outputs, name='citrus_complicated_ensemble_submodel5')
...
def citrus_complicated_ensemble_submodel5_labels_manipulation(labels_array: ndarray) -> int:
    """
