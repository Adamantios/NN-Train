...
def caltech_complicated_ensemble_submodel3(input_shape=None, input_tensor=None, n_classes=None,
    ...
    x = Flatten()(x)
    x = Dense(2048, kernel_regularizer=l2(weight_decay))(x)
    ...
    # Create Submodel 1.
    model = Model(inputs, outputs, name='caltech_complicated_ensemble_submodel3')
...
def caltech_complicated_ensemble_submodel3_labels_manipulation(labels_array: ndarray) -> int:
    """
    ...
    """
    labels_array[logical_or(labels_array < 32, labels_array > 61)] = -1

    for i, label_i in enumerate(range(32, 62)):
