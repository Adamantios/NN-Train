...
def caltech_complicated_ensemble_submodel2(input_shape=None, input_tensor=None, n_classes=None,
    ...
    x = Conv2D(1024, (3, 3), padding='same', activation='relu', kernel_regularizer=l2(weight_decay))(x)
    ...
    # Create Submodel 1.
    model = Model(inputs, outputs, name='caltech_complicated_ensemble_submodel2')
...
def caltech_complicated_ensemble_submodel2_labels_manipulation(labels_array: ndarray) -> int:
    """
    ...
    """
    labels_array[logical_or(labels_array < 22, labels_array > 51)] = -1

    for i, label_i in enumerate(range(22, 52)):
